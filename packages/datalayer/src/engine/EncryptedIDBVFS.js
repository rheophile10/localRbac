...
// VENDORED from @vlcn.io/wa-sqlite/src/examples/IDBBatchAtomicVFS.js.
// The ONLY changes from upstream: the IDBContext import points at our
// EncryptingIDBContext (transparent per-block page encryption at the IndexedDB
// boundary), the class is renamed, and the constructor accepts a 32-byte key.
// Everything else is byte-identical, so re-vendoring on an upstream bump is a
// mechanical diff. See EncryptingIDBContext.js for the encryption seam.
import * as VFS from '@vlcn.io/wa-sqlite/src/VFS.js';
import { WebLocksExclusive as WebLocks } from '@vlcn.io/wa-sqlite/src/examples/WebLocks.js';
import { EncryptingIDBContext as IDBContext } from './EncryptingIDBContext.js';
...
export class EncryptedIDBVFS extends VFS.Base {
  #options;
  ...
  constructor(idbDatabaseName = 'wa-sqlite', key = null, options = DEFAULT_OPTIONS) {
    super();
    this.name = idbDatabaseName;
    this.#options = Object.assign({}, DEFAULT_OPTIONS, options);
    // `key` (32 bytes) enables transparent page encryption at rest; null = off.
